...
const resolvers = {
  Query: {
    ...
    agencies: (_, { status }) =>
      prisma.agency.findMany({ where: status ? { status } : undefined }),
    agent: (_, { id }) => prisma.agent.findUnique({ where: { id } }),
    agents: (_, { status }) =>
      prisma.agent.findMany({ where: status ? { status } : undefined }),
    context: (_, { id }) => prisma.context.findUnique({ where: { id } }),
    contexts: (_, { agencyId }) =>
      prisma.context.findMany({ where: { agencyId } }),
    timeSlot: (_, { id }) => prisma.timeSlot.findUnique({ where: { id } }),
    timeSlots: (_, { agentId, contextId, sessionTypeId }) =>
    ...
    nextTimeSlot: async (_, { contextId, currentTime }) => {
      ...
      timeSlots.forEach((timeSlot) => {
        try {
          ...
          if (nextExecution >= now && (!minTime || nextExecution < minTime)) {
            minTime = nextExecution;
            nextSlot = { ...timeSlot, startTime: nextExecution };
          }
        } catch (e) {
          console.error(
            `Error parsing cron for timeSlot ID ${timeSlot.id}:`,
            e
          );
        }
      });
    ...
    allocation: async (_, { input: { id, timeSlotId, startTime } }) => {
      if (!id && !(timeSlotId && startTime)) {
        throw new Error(
          "Must provide either 'id' or both 'timeSlotId' and 'startTime' to look up an allocation."
        );
      }
      console.log("🔍 [Server] Looking up allocation...", {
        id,
        timeSlotId,
        startTime,
      });
      try {
        ...
        if (id) {
          ...
          console.log("🔍 [Server] Searching by timeSlotId and startTime:", {
            timeSlotId,
            startTime,
          });
    ...
    possibleAllocations: async (_, { contextId, startDate, endDate }) => {
      console.log("🔍 [Server] Fetching possible allocations...", {
        contextId,
        startDate,
        endDate,
      });
      try {
        ...
        const possibleAllocations = timeSlots.flatMap((timeSlot) => {
          ...
          const effectiveDuration =
            timeSlot.duration || timeSlot.sessionType.defaultDuration;
          ...
          console.log(
            "📝 [Server] Generated occurrences count:",
            occurrences.length
          );
          return occurrences.map((startTime) => {
            return {
              id: timeSlot.allocations.find(
                (allocation) =>
                  allocation.startTime.getTime() === startTime.getTime()
              )?.id,
              startTime,
        ...
        console.log(
          "✅ [Server] Total possible allocations:",
          possibleAllocations.length
        );
        ...
        console.error(
          "❌ [Server] Error generating possible allocations:",
          error
        );
        throw error;
      }
    ...
    consumer: (_, { id }) =>
      prisma.consumer.findUnique({ where: { id }, include: { bundles: true } }),
    consumers: async (_, { filters, orderBy }) => {
      const where = {
        isDeleted: filters?.includeDeleted ? undefined : false,
        status:
          filters?.statuses?.length > 0 ? { in: filters.statuses } : undefined,
        tags:
          ...
          OR: [
            ...
            { email: { contains: query, mode: "insensitive" } },
            {
              AND: [
                {
                  firstName: {
                    contains: fullNameQuery.split(" ")[0],
                    mode: "insensitive",
                  },
                },
                {
                  lastName: {
                    contains: fullNameQuery.split(" ")[1] || "",
                    mode: "insensitive",
                  },
                },
              ],
    ...
    groupReservation: (_, { id }) =>
      prisma.groupReservation.findUnique({ where: { id } }),
  ...
  Mutation: {
    ...
    updateAgency: (_, { id, input }) =>
      prisma.agency.update({ where: { id }, data: input }),
    createAgent: (_, { input }) => prisma.agent.create({ data: input }),
    updateAgent: (_, { id, input }) =>
      prisma.agent.update({ where: { id }, data: input }),
    createContext: (_, { input }) => prisma.context.create({ data: input }),
    updateContext: (_, { id, input }) =>
      prisma.context.update({ where: { id }, data: input }),
    createTimeSlot: (_, { input }) => prisma.timeSlot.create({ data: input }),
    updateTimeSlot: (_, { id, input }) =>
      prisma.timeSlot.update({ where: { id }, data: input }),
    ...
    updateAllocation: async (_, { id, input }) => {
      ...
      if (input.status === "CANCELLED") {
        for (const reservation of updatedAllocation.reservations) {
          await handleBundleUsageEvent(
            reservation.bundle.id,
            reservation.id,
            "REFUND",
            1
          );
    ...
    createSessionType: (_, { input }) =>
      prisma.sessionType.create({ data: input }),
    updateSessionType: (_, { id, input }) =>
      prisma.sessionType.update({ where: { id }, data: input }),
        ...
        const result = await prisma.$transaction(async (tx) => {
          ...
          const allocation = await tx.allocation.upsert({
            ...
            create: {
              ...
              startTime: input.startTime,
              endTime: new Date(
                new Date(input.startTime).getTime() + effectiveDuration * 60000
          ...
          console.log(
            "Sesión completa recuperada exitosamente:",
            completeSession
          );
    ...
    updateSession: (_, { id, input }) =>
      prisma.session.update({ where: { id }, data: input }),
    ...
    updateConsumer: (_, { id, input }) =>
      prisma.consumer.update({ where: { id }, data: input }),
...
*Consumer Name:* ${
        bundleWithRelations.consumer.firstName +
          " " +
          bundleWithRelations.consumer.lastName || "N/A"
      }
...
`;
    ...
    updateBundle: (_, { id, input }) =>
      prisma.bundle.update({ where: { id }, data: input }),
    extendBundle: async (_, { bundleId, daysToExtend }) => {
      if (daysToExtend < 1 || daysToExtend > 15) {
        throw new Error(
          "Invalid number of days. Please select between 1 and 15 days."
        );
      }
      ...
      if (bundle.isExtended) {
        throw new Error(
          "This bundle has already been extended and cannot be extended again."
        );
      }
      ...
      const updatedBundle = await prisma.bundle.update({
        ...
        data: {
          validTo: new Date(
            new Date(bundle.validTo).getTime() +
              daysToExtend * 24 * 60 * 60 * 1000
          ), // Add days
      ...
      const result = await prisma.$transaction(
        async (tx) => {
          ...
          if (allocation.status !== "AVAILABLE")
            throw new Error("Allocation is not available");
          ...
          console.log(
            ...
            "| maxConsumers:",
            timeSlot.sessionType.maxConsumers
          );
          if (
            allocation.currentReservations >= timeSlot.sessionType.maxConsumers
          ) {
          ...
          console.log(
            "Buscando reserva previa del usuario:",
            input.forConsumerId,
            " en allocation:",
            allocation.id
          );
          ...
          if (existingReservation)
            throw new Error(
              "User already has a reservation for this allocation"
            );

          // 3. Revisa el bundle usage
          console.log("Antes de llamar a calculateBundleRemainingUses");
          try {
            const remainingUses = await calculateBundleRemainingUses(
              tx,
              input.bundleId
            );
            ...
            console.error(
              "Error dentro de calculateBundleRemainingUses o al obtener remainingUses:",
              err
            );
          ...
          const newReservation = await tx.reservation.create({
            data: {
              ...
              ...(input.forConsumerId
                ? { forConsumer: { connect: { id: input.forConsumerId } } }
                : {}),
              ...(input.onBehalfOfName
                ? { onBehalfOfName: input.onBehalfOfName }
                : {}),
            },
            include: {
          ...
          console.log(
            "Actualizando allocation currentReservations y version..."
          );
    ...
    updateReservation: async (_, { id, input }) => {
      const updatedReservation = await prisma.reservation.update({
        ...
        include: {
          ...
          allocation: {
            include: {
              timeSlot: { include: { sessionType: true, agent: true } },
            },
          },
          forConsumer: true,
        },
      ...
      console.log(
        "Hora",
        sessionStartTime.toLocaleTimeString("es-PE", {
          hour: "2-digit",
          minute: "2-digit",
          hour12: false,
        })
      ...
      console.log(
        "Validated At: ",
        attendanceList.map((item) => item.validatedAt)
      );
    ...
    👨‍🏫 Detalles de Sesión:
    - Profesor: ${
      updatedReservation.allocation?.timeSlot?.agent?.name || "Desconocido"
    }
    - Clase: ${
      updatedReservation.allocation?.timeSlot?.sessionType?.name ||
      "Sesión desconocida"
    }
    - Hora: ${sessionStartTime.toLocaleTimeString("es-PE", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
      timeZone: "America/Lima",
      ...
      const payload = {
        ...
        session: {
          type:
            ...
            "Unknown Session",
          professor:
            updatedReservation.allocation?.timeSlot?.agent?.name ||
            "Unknown Professor",
      ...
      try {
        const result = await prisma.$transaction(
          async (tx) => {
            ...
            if (
              allocation.currentReservations + groupSize >
              allocation.timeSlot.sessionType.maxConsumers
            )
              throw new Error("Not enough slots available for the group");
              ...
              await tx.reservation.create({
                data: {
                  ...
                  ...(input.bundleId
                    ? { bundle: { connect: { id: input.bundleId } } }
                    : {}),
                  ...(participant.forConsumerId
                    ? {
                        forConsumer: {
                          connect: { id: participant.forConsumerId },
                        },
                      }
                    : {}),
                  ...(participant.onBehalfOfName
            ...
            const updatedAllocation = await tx.allocation.update({
              where: {
                ...
                version: allocation.version,
                currentReservations: {
                  lte: allocation.timeSlot.sessionType.maxConsumers - groupSize,
                },
              },
              data: {
    ...
    updateGroupReservation: (_, { id, input }) =>
      prisma.groupReservation.update({ where: { id }, data: input }),
  ...
  Agency: {
    contexts: (parent) =>
      prisma.context.findMany({ where: { agencyId: parent.id } }),
    agentSessionTypeAgencies: (parent) =>
      prisma.agentSessionTypeAgency.findMany({
  ...
  Agent: {
    timeSlots: (parent) =>
      prisma.timeSlot.findMany({ where: { agentId: parent.id } }),
    agentSessionTypeAgencies: (parent) =>
      prisma.agentSessionTypeAgency.findMany({ where: { agentId: parent.id } }),
    sessions: (parent) =>
      prisma.session.findMany({ where: { agentId: parent.id } }),
    altSessionAgents: (parent) =>
      prisma.altSessionAgent.findMany({ where: { agentId: parent.id } }),
  },

  Context: {
    agency: (parent) =>
      prisma.agency.findUnique({ where: { id: parent.agencyId } }),
    parentContext: (parent) =>
      parent.parentContextId
        ? prisma.context.findUnique({ where: { id: parent.parentContextId } })
        : null,
    children: (parent) =>
      prisma.context.findMany({ where: { parentContextId: parent.id } }),
    timeSlots: (parent) =>
      prisma.timeSlot.findMany({ where: { contextId: parent.id } }),
  },
  ...
  TimeSlot: {
    ...
    context: (parent) =>
      prisma.context.findUnique({ where: { id: parent.contextId } }),
    sessionType: (parent) =>
      prisma.sessionType.findUnique({ where: { id: parent.sessionTypeId } }),
    allocations: (parent) =>
      prisma.allocation.findMany({ where: { timeSlotId: parent.id } }),
    duration: async (parent) => {
      if (parent.duration !== null && parent.duration !== undefined) {
        return parent.duration;
      }
      const sessionType = await prisma.sessionType.findUnique({
        where: { id: parent.sessionTypeId },
      });
  ...
  Allocation: {
    sessionType: (parent) =>
      prisma.sessionType.findUnique({ where: { id: parent.sessionTypeId } }),
    timeSlot: (parent) =>
      prisma.timeSlot.findUnique({ where: { id: parent.timeSlotId } }),
    sessions: (parent) =>
      prisma.session.findMany({ where: { allocationId: parent.id } }),
    reservations: (parent) =>
      prisma.reservation.findMany({ where: { allocationId: parent.id } }),
    groupReservations: (parent) =>
      prisma.groupReservation.findMany({ where: { allocationId: parent.id } }),
  },

  SessionType: {
    timeSlots: (parent) =>
      prisma.timeSlot.findMany({ where: { sessionTypeId: parent.id } }),
    agentSessionTypeAgencies: (parent) =>
      prisma.agentSessionTypeAgency.findMany({
        where: { sessionTypeId: parent.id },
      }),
  },

  Session: {
    agent: (parent) =>
      prisma.agent.findUnique({ where: { id: parent.agentId } }),
    allocation: (parent) =>
      prisma.allocation.findUnique({ where: { id: parent.allocationId } }),
    sessionConsumers: (parent) =>
      prisma.sessionConsumer.findMany({ where: { sessionId: parent.id } }),
  },

  Consumer: {
    sessionConsumers: (parent) =>
      prisma.sessionConsumer.findMany({ where: { consumerId: parent.id } }),
    bundles: (parent, { statuses = ["ACTIVE"] }) =>
      prisma.bundle.findMany({
        where: {
    ...
    reservations: (parent) =>
      prisma.reservation.findMany({ where: { forConsumerId: parent.id } }),
    tags: (parent) =>
      prisma.tag.findMany({
        where: { consumers: { some: { id: parent.id } } },
      }),
  ...
  Bundle: {
    bundleType: (parent) =>
      prisma.bundleType.findUnique({ where: { id: parent.bundleTypeId } }),
    consumer: (parent) =>
      prisma.consumer.findUnique({ where: { id: parent.consumerId } }),
    parentBundle: (parent) =>
      parent.parentBundleId
        ? prisma.bundle.findUnique({ where: { id: parent.parentBundleId } })
        : null,
    children: (parent) =>
      prisma.bundle.findMany({ where: { parentBundleId: parent.id } }),
    reservations: (parent) =>
      prisma.reservation.findMany({ where: { bundleId: parent.id } }),
    ...
    bundleUsageEvents: (parent) =>
      prisma.bundleUsageEvent.findMany({ where: { bundleId: parent.id } }),
  },
  ...
  BundleUsageEvent: {
    ...
    reservation: (parent) =>
      prisma.reservation.findUnique({ where: { id: parent.reservationId } }),
  },
  ...
  Reservation: {
    ...
    allocation: (parent) =>
      prisma.allocation.findUnique({ where: { id: parent.allocationId } }),
    forConsumer: (parent) =>
      parent.forConsumerId
        ? prisma.consumer.findUnique({ where: { id: parent.forConsumerId } })
        : null,
    groupReservation: (parent) =>
      parent.groupReservationId
        ? prisma.groupReservation.findUnique({
            where: { id: parent.groupReservationId },
          })
        : null,
    bundleUsageEvents: (parent) =>
      prisma.bundleUsageEvent.findMany({ where: { reservationId: parent.id } }),
  },
  ...
  GroupReservation: {
    allocation: (parent) =>
      prisma.allocation.findUnique({ where: { id: parent.allocationId } }),
    reservations: (parent) =>
      prisma.reservation.findMany({ where: { groupReservationId: parent.id } }),
  },
